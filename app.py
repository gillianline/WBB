...
with active_season:
    ...
    # =========================================================================
        ...
        st.markdown(
            '<div class="vball-section-title">Live Performance Event Tracker</div>',
            unsafe_allow_html=True,
        )

        # Context Controls: Session Week & Day
        col_w, col_d, _ = st.columns([1, 1, 2])
        with col_w:
            week_starting = st.date_input("Week Starting Date:", value=pd.to_datetime("today"))
        with col_d:
            day_selected = st.selectbox("Session Day:", ["Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday", "Sunday"])

        # Fetch and sync persistent counts
        init_live_track_state(roster_players, week_starting, day_selected)

        st.markdown("<br>", unsafe_allow_html=True)

        # Render Player Cards in 2 Columns
        metrics_list = ["Box Out", "Turnovers", "Offensive Rebounds"]
        ...
        for i in range(0, len(roster_players), 2):
            ...
            for j in range(2):
                if i + j < len(roster_players):
                    ...
                    with cols[j]:
                        with st.container():
                            # Athlete Card Profile Header
                            st.markdown(
                                f"""
                                <div style="background:#FFFFFF; border:1px solid #CBD5E1; border-radius:10px 10px 0 0; padding:12px 16px; display:flex; align-items:center; gap:12px; border-bottom:2px solid #F1F5F9;">
                                    <img src="{p_img}" class="athlete-avatar" style="width:50px; height:50px;">
                                    <div>
                                        <h4 style="margin:0; font-size:1.1rem; color:#0F172A;">{player_name}</h4>
                                        <span style="color:#64748B; font-size:0.8rem;">{p_pos}</span>
                            ...
                            # Metrics Counter Controls Container
                            st.markdown('<div style="background:#FFFFFF; border:1px solid #CBD5E1; border-top:none; border-radius:0 0 10px 10px; padding:12px 16px; margin-bottom:20px;">', unsafe_allow_html=True)

                            for metric in metrics_list:
                                current_val = st.session_state["live_counts"][player_name][metric]
                                c_m_label, c_btn_minus, c_val, c_btn_plus = st.columns([2.5, 1, 1, 1])
