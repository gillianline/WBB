...
# -----------------------------------------------------------------------------
# 7. SEASON NAVIGATION (DEFAULTS TO PRE-SEASON ON LOGIN)
# -----------------------------------------------------------------------------
season_options = ["Summer", "Pre-Season", "Combined Seasons", "Team Wellness"]

if "active_season_tab" not in st.session_state:
    st.session_state["active_season_tab"] = "Pre-Season"

# Segmented control style to render buttons that look and act like tabs
st.markdown(
    """
    <style>
...
current_tab = st.radio(
    ...
    index=season_options.index(st.session_state["active_season_tab"]),
...
# -----------------------------------------------------------------------------
# 8. DASHBOARD RENDER ENGINE PER SEASON
# -----------------------------------------------------------------------------
        ...
        for player_name in roster_players:
            ...
            <div class="practice-score-card" style="background-color: #FFFFFF; border: 1px solid #CBD5E1; border-radius: 12px; padding: 16px 20px; margin-bottom: 20px; box-shadow: 0 2px 6px rgba(0,0,0,0.04);">
                <div style="display: flex; align-items: center; justify-content: space-between; margin-bottom: 12px; border-bottom: 1px solid #E2E8F0; padding-bottom: 8px;">
                    ...
                    <div style="display: flex mechanical gap: 6px; align-items: center;">
                        <span style="background:#F1F5F9; border:1px solid #E2E8F0; color:#475569; padding:3px 8px; border-radius:6px; font-weight:600; font-size:0.75rem;">Minutes: {mins}</span>
                        <span style="background:#F1F5F9; border:1px solid #E2E8F0; color:#475569; padding:3px 8px; border-radius:6px; font-weight:600; font-size:0.75rem;">Week {wk_str}</span>
                        <span style="background:#F1F5F9; border:1px solid #E2E8F0; color:#475569; padding:3px 8px; border-radius:6px; font-weight:600; font-size:0.75rem;">Day {dy_str}</span>
                        <span style="background:#EFF6FF; border:1px solid #BFDBFE; color:#1D4ED8; padding:3px 8px; border-radius:6px; font-weight:700; font-size:0.75rem;">{sess_type}</span>
...
# -----------------------------------------------------------------------------
# 9. COMBINED SEASONS DASHBOARD RENDER ENGINE
# -----------------------------------------------------------------------------
def render_combined_seasons_content():
    ...
    if not df_comb_timeline.empty:
        ...
        def create_custom_timeline_chart(metric_col, title_label):
            ...
            # Solid black line connecting all points chronologically
            fig.add_trace(
                go.Scatter(
            ...
            # Circular points styled by Session Type
