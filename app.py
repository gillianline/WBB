...
with active_season:
    ...
    # =========================================================================
    # TAB 2: PRACTICE SCORE (Fixed HTML String Escaping)
    # =========================================================================
        ...
        for player_name in roster_players:
            ...
            # Clean metadata strings to avoid duplicate "Week Week 3" text
            wk_str = str(wk).replace("Week ", "")
            dy_str = str(dy).replace("Day ", "")

            # Single unbroken HTML block with NO comments or markdown indentations
            single_box_card_html = (
                f'<div style="background-color: #FFFFFF; border: 1px solid #CBD5E1; border-radius: 12px; padding: 20px; margin-bottom: 25px; box-shadow: 0 2px 6px rgba(0,0,0,0.04);">'
                f'<div style="display: flex; align-items: center; justify-content: space-between; margin-bottom: 18px; border-bottom: 1px solid #E2E8F0; padding-bottom: 12px;">'
                f'<div style="display: flex; align-items: center; gap: 15px;">'
                f'<img src="{p_img}" style="width:60px; height:60px; border-radius:50%; border:3px solid #FF8200; object-fit:cover;">'
                f'<div><h3 style="margin:0; font-size:1.3rem; color:#0F172A; font-weight:700;">{player_name}</h3><span style="color:#64748B; font-size:0.85rem;">{p_pos}</span></div>'
                f'</div>'
                f'<div style="display: flex; gap: 8px;">'
                f'<span style="background:#F1F5F9; border:1px solid #E2E8F0; color:#475569; padding:4px 10px; border-radius:6px; font-weight:600; font-size:0.8rem;">Minutes: {mins}</span>'
                f'<span style="background:#F1F5F9; border:1px solid #E2E8F0; color:#475569; padding:4px 10px; border-radius:6px; font-weight:600; font-size:0.8rem;">Week {wk_str}</span>'
                f'<span style="background:#F1F5F9; border:1px solid #E2E8F0; color:#475569; padding:4px 10px; border-radius:6px; font-weight:600; font-size:0.8rem;">Day {dy_str}</span>'
                f'</div>'
                f'</div>'
                f'<div style="display: flex; gap: 20px; width: 100%;">'
                f'<div style="flex: 1; min-width: 0;">'
                f'<div style="background-color:#38BDF8; color:#0F172A; font-weight:700; font-size:0.95rem; padding:6px 12px; border-radius:6px; text-align:center; margin-bottom:12px; text-transform:uppercase;">Volume Metrics</div>'
                f'{vol_html_table}'
                f'<div style="background:#FFFFFF; border:1px solid #E2E8F0; border-radius:8px; padding:10px; text-align:center; margin-top:10px;">'
                f'<div style="font-weight:700; color:#64748B; font-size:0.85rem;">VOLUME SCORE</div>'
                f'<div style="font-size:2rem; font-weight:800; padding:6px 0; border-radius:6px; background-color:{v_bg}; color:{v_fg}; margin-top:4px;">{vol_score}</div>'
                f'</div>'
                f'</div>'
                f'<div style="flex: 1; min-width: 0;">'
                f'<div style="background-color:#38BDF8; color:#0F172A; font-weight:700; font-size:0.95rem; padding:6px 12px; border-radius:6px; text-align:center; margin-bottom:12px; text-transform:uppercase;">Intensity Metrics</div>'
                f'{int_html_table}'
                f'<div style="background:#FFFFFF; border:1px solid #E2E8F0; border-radius:8px; padding:10px; text-align:center; margin-top:10px;">'
                f'<div style="font-weight:700; color:#64748B; font-size:0.85rem;">INTENSITY SCORE</div>'
                f'<div style="font-size:2rem; font-weight:800; padding:6px 0; border-radius:6px; background-color:{i_bg}; color:{i_fg}; margin-top:4px;">{int_score}</div>'
                f'</div>'
                f'</div>'
                f'</div>'
                f'</div>'
